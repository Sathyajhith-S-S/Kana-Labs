...
const NavLink = ({ to, children, disabled = false, onClick }: { to: string; children: React.ReactNode; disabled?: boolean; onClick?: () => void; }) => {
    const location = useLocation();
    const active = location.pathname === to;
    const classes = "flex items-center gap-3 md:gap-1 px-3 py-2 text-sm md:text-xs font-bold rounded-md transition-all duration-200 hover:bg-muted";

    if (disabled) {
        return <span className={`${classes} text-muted-foreground/50 cursor-not-allowed`}>{children}</span>;
    }

    return (
        <Link to={to} onClick={onClick} className={`${classes} ${active ? 'text-primary' : 'text-muted-foreground hover:text-primary hover:-translate-y-0.5'}`}>
            {children}
        </Link>
    );
};
...
export function Header() {
    ...
    return (
        <TooltipProvider>
            <header className="fixed top-0 left-0 right-0 bg-card backdrop-blur-sm z-50 border-b border-black/10 dark:border-white/10">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="hidden md:flex items-center">
                                <NavLink to="/swap" onClick={closeMenu}>
                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" transform="rotate(90)"><path d="M8 18V6l-4 4" /><path d="M16 6v12l4-4" /></svg>
                                    Swap
                                </NavLink>
                                ...
                                <NavLink to="/perps" onClick={closeMenu}>
                        ...
                        <div className="flex items-center gap-3">
                            <div className="hidden md:flex items-center gap-3">
                                {location.pathname === '/perps' ? (
                                    <>
                                        <Button variant="secondary" className="px-2 flex items-center border border-black/10 dark:border-white/10 gap-2">
                                            <FontAwesomeIcon icon={faStar} style={{ color: "#989690" }} size="sm" />
                                            <span className="font-bold text-xs" style={{ color: "#989690" }}>VIP</span>
                                            <Tooltip><TooltipTrigger asChild><FontAwesomeIcon icon={faCircleInfo} style={{ color: "#FFD43B" }} size="sm" /></TooltipTrigger><TooltipContent><p>Coming soon</p></TooltipContent></Tooltip>
                                        </Button>
                            ...
                            <Button variant="secondary" size="lg" className="p-2 h-auto group border border-black/10 dark:border-white/10">
                                <svg className="group-hover:animate-ring" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.73 21a2 2 0 0 1-3.46 0" /></svg>
                            </Button>
                            <Button onClick={toggleTheme} variant="secondary" size="sm" className="p-2 h-auto border border-black/10 dark:border-white/10">
                                {theme === 'light' ? <svg className="hover:scale-125" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"></path></svg> : <svg className="hover:scale-125" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="5"></circle><line x1="12" y1="1" x2="12" y2="3"></line><line x1="12" y1="21" x2="12" y2="23"></line><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"></line><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"></line><line x1="1" y1="12" x2="3" y2="12"></line><line x1="21" y1="12" x2="23" y2="12"></line><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"></line><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"></line></svg>}
                            </Button>

                            <div className="md:hidden">
                                <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 rounded-md text-muted-foreground">
                                    {isMenuOpen ? <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="red"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg> : <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16m-7 6h7" /></svg>}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                {isMenuOpen && (
                    <div className="md:hidden bg-card border-b-4 border-t-4 border-black/10 dark:border-white/10 shadow-2xl">
                        <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                            ...
                            <div className="pt-4 border-t border-black/10 dark:border-white/10">
                                {location.pathname === '/perps'
                                    ? <Button variant="secondary" className="w-full flex items-center border border-black/10 dark:border-white/10 gap-2 text-primary hover:animate-bounce-subtle">
                                        <FontAwesomeIcon icon={faCircleUser} />
